...
class DuplicateFinderGUI:
    ...
    def create_widgets(self):
        ...
        title_label = ttk.Label(
            main_frame, text="Simple Duplicate Finder", font=("Helvetica", 14, "bold"))
        ...
        self.folder_entry = ttk.Entry(
            folder_frame, textvariable=self.selected_folder, width=40)
        ...
        self.scan_btn = ttk.Button(
            main_frame, text="Start Scan", command=self.start_scan)
        ...
        self.show_res_btn = ttk.Button(
            main_frame, text="Show Results", command=self.open_results_window, state=tk.DISABLED)
    ...
    def run_scan_thread(self, folder):
        try:
            # Run the scan
            self.duplicates = scan_for_duplicates(folder)
...
class ResultsWindow:
    ...
    def create_ui(self):
        ...
        ttk.Button(top_frame, text="Delete All (Keep One per Group)",
        ...
        ttk.Button(top_frame, text="Close",
    ...
    def populate_list(self):
        for i, group in enumerate(self.duplicates):
            ...
            group_frame = ttk.LabelFrame(
                self.scrollable_frame, text=f"Group {i+1} - Size: {size} bytes", padding=5)
    ...
    def delete_selected(self):
        to_delete = [path for path, var in self.check_vars.items()
                     if var.get() == 1]
